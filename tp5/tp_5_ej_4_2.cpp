...
// D0 = Cut Frecuency
CImgList<double> butterworth(int width, int height, int D0, int order){
  ...
  cimg_forXY(filter[0], x, y){
    ...
    filter[0](x,y) = 1 / ( 1 + pow(factor/float(D0),  2*order));
  }
...
int main(int argc, char *argv[]) {
  ...
  CImgList<double> filter = butterworth(img.width(), img.height(), D0, order);

  CImg<double> img_filter = frequency_filter(img, filter);

  CImgDisplay vent_1(img, "Image");
  CImgDisplay vent_2(img_filter, "Image with Butterworth Filter");
